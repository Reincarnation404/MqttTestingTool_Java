...
//    String clientId;
    ...
    * 訂閱主題
    ...
    public void subscribeToTopic(String topic, String qos) {
        try {
            mqttAndroidClient.subscribe(topic, Integer.parseInt(qos), null, new IMqttActionListener() {
                @Override
                public void onSuccess(IMqttToken asyncActionToken) {
     ...
     * @param message 訊息內容
     */
    public void publishToTopic(String topic,String message, String qos, boolean retained){
        MqttMessage m = new MqttMessage();
                    //string轉byte[]
        m.setPayload(message.getBytes());
        //設定QoS
        m.setQos(Integer.parseInt(qos));
        //設定是否保留
    ...
    public boolean isConnected(){
        boolean connection;
        connection = mqttAndroidClient.isConnected();
        return connection;
    }
